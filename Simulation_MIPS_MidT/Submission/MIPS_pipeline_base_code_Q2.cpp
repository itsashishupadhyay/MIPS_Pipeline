...
int main()
{
    ...
    while (1) {
        ...
        {                       
            ...
            else if (state.EX.is_I_type==1 && state.EX.Wrt_reg_addr==0 && state.EX.alu_op==1 && state.EX.wrt_enable==0 && state.EX.wrt_mem ==0)
            { // Beq 
             if (state.EX.Read_data1 != state.EX.Read_data2)
                {
                    ...
                    // newState.IF.PC=bitset<32> ( state.IF.PC.to_string().substr(0, 4)+ state.EX.Imm.to_string().substr(6, 26)+ "00");
        ...
        cycle ++;	
       // cout<<"Real Time Cycle "<<cycle<<endl;
        
        // cout<<"if "<<state.IF.nop<<" ID "<<state.ID.nop<<" EX "<<state.EX.nop<<" MEM "<<state.MEM.nop<<endl;
